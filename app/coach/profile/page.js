...
import { Camera, Loader2, ArrowLeft, Save, ShieldCheck } from "lucide-react";
...
export default function CoachProfilePage() {
  ...
  // Upload de l'avatar vers le bucket Supabase Storage "avatars"
  const handleAvatarUpload = async (e) => {
    try {
      ...
      // 1. Envoi dans le bucket "avatars"
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      if (error) throw error;
      alert("Profil coach mis à jour avec succès !");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-4 sm:p-6 max-w-xl mx-auto pb-24">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl mb-6">
        <span className="text-[10px] font-bold uppercase tracking-wider text-amber-400 bg-amber-400/10 px-2.5 py-1 rounded-full border border-amber-400/20 inline-flex items-center gap-1">
          <ShieldCheck className="w-3 h-3" /> Espace Coach
        </span>
        <h1 className="text-xl font-black text-white mt-2">Mon Profil Coach</h1>
        <p className="text-xs text-slate-400 mt-1">
          Gérez votre photo de profil et vos informations visibles par vos élèves.
        </p>
      </div>
