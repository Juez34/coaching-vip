...
const fetchWorkoutData = async () => {
  ...
  <header className="px-6 pt-8 pb-6 bg-slate-900 border-b border-slate-800 sticky top-0 z-10 backdrop-blur-md bg-slate-900/90 mb-6">
  <div className="flex justify-between items-start gap-4">
    <div>
      ...
      <p className="text-xs text-slate-400 mt-0.5">Suivi de tes séances et entraînements.</p>
    </div>

    <button
      onClick={handleLogout}
      className="text-xs font-bold text-slate-400 hover:text-rose-400 bg-slate-950 hover:bg-slate-800 px-3.5 py-2 rounded-xl border border-slate-800 flex items-center gap-2 transition-colors shrink-0"
      title="Déconnexion"
    >
